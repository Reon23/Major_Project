"""
active_inference_5.py  —  Ryu SDN Controller with Proper Active Inference
==========================================================================

Upgrades over active_inference_4.py
-------------------------------------
1.  PathBelief now implements a *correct* Laplace free energy:
        F = prediction_error + KL_complexity
      = (obs - mu)^2 / (2*sigma_obs^2)  +  (mu - prior)^2 / (2*sigma_prior^2)

2.  Gaussian generative model with an explicit transition function:
        predict_after_load_added(delta)   ->  E[mu | traffic added]
        predict_after_load_removed(delta) ->  E[mu | traffic removed]

3.  Expected Free Energy (EFE) computed per policy (stay / switch):
        G(pi) = extrinsic_value (deviation from preferred utilisation)
              + epistemic_penalty (uncertainty on the target path)

4.  Confidence tracking — sigma_obs grows for the idle path (uncertainty
    inflates when unobserved) and shrinks for the active path.

5.  Softmax policy selection replaces the hard threshold:
        P(switch) = exp(-gamma * G(switch)) / Z
    Action is chosen probabilistically; set EFE_TEMPERATURE high (>=10)
    for near-greedy behaviour.

Everything else (OpenFlow helpers, topology, ARP learning, flow install,
monitor loop) is unchanged from active_inference_4.py.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  PathBelief — Gaussian generative model over link utilisation
# ─────────────────────────────────────────────────────────────────────────────


class PathBelief:
    """
    Gaussian belief over link utilisation for one path.
    ...
    -----------------------------------
        mu <-- mu + alpha * (obs - mu)   (gradient descent on F)
    ...
    # Bounds on sigma_obs so it never collapses to zero or explodes
    SIGMA_OBS_MIN = 0.04
    SIGMA_OBS_MAX = 0.35
    ...
    def __init__(
        ...
        self.sigma_prior = sigma_prior  # width of preference
        self.sigma_obs = sigma_obs  # measurement noise (adaptive)
        self._alpha = alpha
        self.mu = prior  # belief mean, initialised at prior
        self._last_obs = prior  # last raw observation (needed for F)
    ...
    def update(self, observation: float):
        ...
        Equivalent to a Kalman-like update under fixed sigma_obs.
        """
    ...
    def free_energy(self) -> float:
        """
        Full Laplace variational free energy:
            F = prediction_error + KL_complexity
        """
    ...
    # ── Transition model  (generative model, action-conditioned) ─────────────
    ...
    def predict_after_load_removed(self, delta: float) -> float:
        """Expected mu if h1<->h2 load delta leaves this path."""
...
# ─────────────────────────────────────────────────────────────────────────────
#  State export — path for the JSON file consumed by visualizer.py
# ─────────────────────────────────────────────────────────────────────────────
...
class ActiveInferenceLB(app_manager.RyuApp):
    ...
    # Inter-switch port constants
    S1_UPLINKS = [2, 3]  # s1: port2 -> s2, port3 -> s3
    S2_S1_PORT = 3
    S2_S4_PORT = 4
    S3_S1_PORT = 3
    S3_S4_PORT = 4
    ...
    # Bottleneck capacities (Mbps) — must match topology.py
    PATH_BW_MBPS = {0: 10.0, 1: 10.0}

    # ── Active Inference parameters ───────────────────────────────────────────
    POLL_INTERVAL = 2  # seconds between port-stat polls

    # Expected Free Energy temperature γ — higher = more greedy.
    # At γ=8 the agent switches only when G(switch) is clearly lower.
    # Lower γ (e.g. 2) makes policy more exploratory / probabilistic.
    EFE_TEMPERATURE = 8.0

    # Fraction of active-path utilisation attributable to h1<->h2 traffic.
    # Used in the transition model when predicting post-reroute utilisation.
    # Estimated dynamically each cycle; this is the initial / fallback value.
    LOAD_TRANSFER_FRACTION = 0.6

    # Minimum probability of switching before we bother installing flows.
    # Avoids flapping when G(stay) ≈ G(switch).
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._ip_to_mac = {}  # ip   -> mac  (learned from ARP)
        self._mac_to_ip = {}  # mac  -> ip
        self._static_installed = False
        self._lock = threading.Lock()

        # Active path index: 0 = via s2, 1 = via s3
        self._active_path = 0

        # One PathBelief per bottleneck path
        self._beliefs = {0: PathBelief(), 1: PathBelief()}

        # Port-stat byte / time accumulators
        self._last_bytes = defaultdict(int)
        self._last_time = {}

        # Running estimate of h1<->h2 contribution to the active path
    ...
    def _push_h1h2_flows(self, path_idx: int):
        """
        (Re-)install h1<->h2 flows across s1 -> relay(s2 or s3) -> s4.
        Deletes existing priority-20 h1<->h2 rules before reinstalling.
        """
        ...
        relay_s1p = self.S2_S1_PORT  # port3 on both s2 and s3
        relay_s4p = self.S2_S4_PORT  # port4 on both s2 and s3
    ...
    def port_stats_reply_handler(self, ev):
        ...
        # ── Step 1: Perception — update beliefs and confidence ────────────────
        with self._lock:
        ...
        # Confidence: active path is observed -> more certain; idle path -> less certain
        self._beliefs[cur].reinforce_confidence()
        self._beliefs[alt].decay_confidence()

        # ── Step 2: Estimate h1<->h2 load contribution ────────────────────────
        # The idle path carries only background traffic (h3/h4 or h5/h6).
        # The excess on the active path is approximately the h1<->h2 load.
        cur_mu = self._beliefs[cur].mu
        alt_mu = self._beliefs[alt].mu
        h1h2_load = max(0.0, cur_mu - alt_mu)
        # Smooth the estimate to avoid reaction to transient spikes
        self._h1h2_load_estimate = 0.6 * self._h1h2_load_estimate + 0.4 * h1h2_load
        h1h2 = self._h1h2_load_estimate

        # ── Step 3: Expected Free Energy per policy ───────────────────────────
        G = self._compute_efe(cur, alt, h1h2)

        # ── Step 4: Softmax policy selection ─────────────────────────────────
        ...
        # ── Step 5: Act — reroute if inference favours switch ─────────────────
        if not self._static_installed:
            # Still write state so the GUI shows live beliefs even before flows
            self._write_state(cur, h1h2, g_stay, g_switch, p_stay, p_switch,
                              "Waiting for flows to be installed")
            return
        ...
        if chosen_action == 1 and p_switch >= self.SWITCH_PROB_THRESHOLD:
            ...
            event = f"REROUTED h1<->h2: path{cur} -> path{alt}  (P_switch={p_switch:.3f})"
        else:
            event = f"Stayed on path {cur}  (P_stay={p_stay:.3f})"

        # ── Write state.json for the GUI visualizer ───────────────────────────
        with self._lock:
            active_now = self._active_path
        self._write_state(active_now, h1h2, g_stay, g_switch, p_stay, p_switch, event)

    # ── State export — writes state.json for visualizer.py ───────────────────
    ...
    def _write_state(
        ...
        """
        Atomically write the current inference state to STATE_JSON_PATH.
        Uses a temp-file + os.replace so the GUI never reads a partial file.
        """
        state = {
            "timestamp": datetime.datetime.now().isoformat(timespec="seconds"),
            "active_path": active_path,
            "path0": {
                "util":      round(self._beliefs[0].utilisation, 4),
                "mu":        round(self._beliefs[0].mu, 4),
                "F":         round(self._beliefs[0].free_energy, 5),
                "sigma_obs": round(self._beliefs[0].sigma_obs, 4),
            },
            "path1": {
                "util":      round(self._beliefs[1].utilisation, 4),
                "mu":        round(self._beliefs[1].mu, 4),
                "F":         round(self._beliefs[1].free_energy, 5),
                "sigma_obs": round(self._beliefs[1].sigma_obs, 4),
            },
            "h1h2_load": round(h1h2, 4),
            "G_stay":    round(g_stay, 5),
            "G_switch":  round(g_switch, 5),
    ...
    def _compute_efe(self, cur: int, alt: int, h1h2_load: float) -> dict:
        """
        Compute Expected Free Energy G for two policies:
            action 0 (stay):   keep h1<->h2 on path `cur`
            action 1 (switch): move h1<->h2 to path `alt`

        G(pi) = extrinsic_value + epistemic_penalty

        Extrinsic value
        ---------------
        How far the *predicted* post-action utilisation of each path is
        from the preferred utilisation (prior).  Lower = better.

            extrinsic = sum over paths of:
                (predicted_mu - prior)^2 / (2 * sigma_prior^2)

        This is the expected KL complexity under the transition model.

        Epistemic penalty
        -----------------
        Switching to a path with high sigma_obs is risky — we are
        uncertain about its true state.  We add sigma_obs of the
        *target* path as a penalty so the agent is cautious about
        switching to poorly-characterised paths.

        Parameters
        ----------
        cur      : index of current active path (0 or 1)
        alt      : index of alternate path (1 - cur)
        h1h2_load: estimated utilisation fraction due to h1<->h2 traffic
        """
        ...
        # No epistemic penalty for staying — we already know the active path
        epistemic_stay = 0.0
        ...
        # Epistemic penalty: alt path is less certain after being idle
    ...
    # ─────────────────────────────────────────────────────────────────────────
    #  Topology-aware best-effort routing (controller path only)
    # ─────────────────────────────────────────────────────────────────────────
